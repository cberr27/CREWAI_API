...
class DataCrew():
	...
	@agent
	def csv_file_writer(self) -> Agent:
		return Agent(
			config=self.agents_config['csv_file_writer'],
			tools=[FileWriterTool()],
			verbose=True
		)
	@agent
	def json_file_writer(self) -> Agent:
		return Agent(
			config=self.agents_config['json_file_writer'],
			tools=[FileWriterTool()],
			verbose=True
		)
	...
	def data_fetch_task(self) -> Task:
		return Task(
			config=self.tasks_config['data_fetch_task'],
		)

	@task
	def csv_file_write_task(self) -> Task:
		return Task(
			config=self.tasks_config['csv_file_write_task'],
		)

	@task
	def json_file_write_task(self) -> Task:
		return Task(
			config=self.tasks_config['json_file_write_task'],
		)
